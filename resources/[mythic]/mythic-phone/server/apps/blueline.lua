...
function LoadPDTracks(updateClients)
  Logger:Trace('Phone:Blueline', 'Loading PD Tracks')
  local fetchRaceTracks = MySQL.query.await('SELECT * FROM `tracks_pd`', {})
  if #fetchRaceTracks == 0 then
    Logger:Trace('Phone:Blueline', 'No have been loaded for PD Tracks.')
    fetchedTracks = true
    goto continue
  end
  ...
  Logger:Info("Phone:Blueline", string.format("Loading ^2%s^7 PD Racing Tracks", #_tracks))
