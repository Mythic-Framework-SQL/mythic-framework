...
local _deps = {}
if IsDuplicityVersion() then _req = { 'ExportsReady' } end
    ...
    if COMPONENTS[component]._protected then
      ...
      if COMPONENTS[component]._required and #COMPONENTS[component]._required > 0 then
        for k, v in ipairs(COMPONENTS[component]._required) do
          if data[v] == nil then
            COMPONENTS.Logger:Warn('Proxy',
              string.format("Attempt To Extend Component While Missing Required Attribute: ^2%s^7", tostring(v)),
              { console = true })
            return
          end
        end
...
exports('ExtendComponent', function(component, data)
  if COMPONENTS[component] ~= nil then
    if COMPONENTS[component]._protected == nil then
      ...
      COMPONENTS.Logger:Warn('Proxy', string.format("Attempt To Extend Protected Component: ^2%s^7", tostring(component)),
        { console = true })
    end
  else
    COMPONENTS.Logger:Warn('Proxy',
