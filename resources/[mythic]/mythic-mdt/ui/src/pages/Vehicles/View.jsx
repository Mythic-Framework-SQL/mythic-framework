...
import { List, ListItem, ListItemText, Grid, IconButton, Alert } from '@mui/material';
			...
			>
				...
				<Grid item xs={6}>
					<List>
						...
						<ListItem>
							<ListItemText primary="Make / Model" secondary={`${vehicle.Make} ${vehicle.Model}`} />
						</ListItem>
						<ListItem>
						...
						{vehicle.Owner?.Type === 0 ? (
							<ListItem>
								<ListItemText
									...
									secondary={
										<Link className={classes.link} to={`/people?person=${vehicle.Owner?.Id}`}>
						...
						<ListItem>
							<ListItemText primary="Impounded" secondary={vehicle.Storage?.Type === 0 ? 'Yes' : 'No'} />
						</ListItem>
					</List>
				</Grid>
				<Grid item xs={6}>
					...
					{myJob.Id == 'police' && (
						<ListItem>
							<ListItemText
								...
								secondary={vehicle.Strikes?.length == 0 ? 'No Strikes' : null}
							/>
						</ListItem>
					)}
					...
					<ListItem>
						<ListItemText
							...
							secondary={
								<span className={classes.newLineStuff}>
									{vehicle.OwnerHistory
										? vehicle.OwnerHistory.map((o) => {
												if (o.Type == 0) {
													if (o.First) {
														return `Transferred From State ID: ${o.Id} (${o.First} ${
															o.Last
														}) on ${moment(o.Time * 1000).format('LLLL')}`;
													} else {
					...
					{vehicle.PreviousPlates && (
						<ListItem>
							<ListItemText
								primary={'License Plate History'}
								secondary={
									vehicle.PreviousPlates
										? vehicle.PreviousPlates.map((o) => {
												return (
													<span>
														{`${o.oldPlate} -> ${o.newPlate} on ${moment(
															o.time * 1000,
														).format('LLLL')}`}
														<br></br>
													</span>
